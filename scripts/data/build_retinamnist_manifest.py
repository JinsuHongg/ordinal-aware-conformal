...
def main():
 p=argparse.ArgumentParser(description="Regenerate a RetinaMNIST split assignment for audit only.")
 p.add_argument("--npz",type=Path,required=True)
 p.add_argument("--output",type=Path,default=Path("data/split_assignments/conference_v0_3/retinamnist"))
 p.add_argument("--seed",type=int,default=0); p.add_argument("--overwrite",action="store_true"); a=p.parse_args()
